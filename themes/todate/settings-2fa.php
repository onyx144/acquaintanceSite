...
<div class="row to_sett_row">
	...
	<div class="col m12 l8">
		<form method="post" action="/profile/save_twofactor_setting" class="dt_settings" id="twofactor_settings">
			<h2 class="user_sttng_panel_hd"><?php echo __( 'Two-factor authentication' );?></h2>
			<div class="delete_acc">
                <p><?php echo __("Turn on 2-step login to level-up your account's security, Once turned on, you'll use both your password and a 6-digit security code sent to your phone or email to log in.");?></p>
                <hr class="border_hr">
            </div>
			...
			<div class="row">
				<?php if ($config->two_factor_type == 'both' || $config->two_factor_type == 'phone') { ?>
                    <div class="col l6">
						<div class="to_mat_input">
							<input name="phone_number" id="phone_number" class="browser-default" type="text" placeholder="<?php echo __( 'Phone' );?>" value="<?php echo $profile->phone_number?>">
				...
				<?php if ($config->two_factor_type == 'both' || $config->two_factor_type == 'email') { ?>
                    <div class="col l6">
						<div class="to_mat_input">
							<input name="new_email" id="new_email" class="browser-default" type="text" placeholder="<?php echo __( 'Email' );?>" value="<?php echo $profile->email?>">
							<label for="new_email"><?php echo __( 'Email' );?></label>
						</div>
                    </div>
				<?php } ?>
			</div>
			<div class="dt_sett_footer">
                <button class="btn btn-large bold btn_primary btn_round" type="button" name="action" id="save_form"><span><?php echo __( 'Save' );?></span> <svg viewBox="0 0 19 14" xmlns="http://www.w3.org/2000/svg" width="18" height="18"><path fill="currentColor" d="M18.6 6.9v-.5l-6-6c-.3-.3-.9-.3-1.2 0-.3.3-.3.9 0 1.2l5 5H1c-.5 0-.9.4-.9.9s.4.8.9.8h14.4l-4 4.1c-.3.3-.3.9 0 1.2.2.2.4.2.6.2.2 0 .4-.1.6-.2l5.2-5.2h.2c.5 0 .8-.4.8-.8 0-.3 0-.5-.2-.7z"></path></svg></button>
            </div>
...
<div id="modal_confirmation" class="modal">
    <div class="modal-content">
        <h4><?php echo __( 'A confirmation email has been sent.' );?></h4>
        <p><?php echo __( 'We have sent an email that contains the confirmation code to enable Two-factor authentication.' );?></p>
		<div class="to_mat_input">
			<input name="code" id="twofactor_confirmationcode" class="browser-default" type="text" placeholder="<?php echo __( 'Confirmation code' );?>">
			<label for="twofactor_confirmationcode"><?php echo __( 'Confirmation code' );?></label>
		</div>
    </div>
    <div class="modal-footer">
        <button id="send_confirmation_btn" data-userid="<?php echo $profile->id;?>" class="btn-flat btn_primary white-text"><?php echo __( 'Confirm' );?></button>
    </div>
</div>
